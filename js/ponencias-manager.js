...
class PonenciasManager {
    ...
    generarRecesoHTML(ponencia) {
        return `
            <div class="flex items-center space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-yellow-500">
                ...
                <div class="flex-shrink-0">
                    <div class="h-28 w-28 rounded-full bg-yellow-100 flex items-center justify-center">
    ...
    generarAlmuerzoHTML(ponencia) {
        return `
            <div class="flex items-center space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-orange-500">
                <div class="text-center w-24 flex-shrink-0">
                ...
                <div class="flex-shrink-0">
                    <div class="h-28 w-28 rounded-full bg-orange-100 flex items-center justify-center">
    ...
    generarEventoHTML(ponencia) {
        return `
            <div class="flex items-center space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-gray-400">
                <div class="text-center w-24 flex-shrink-0">
    ...
    generarClausuraHTML(ponencia) {
        return `
            <div class="flex items-center space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-red-500">
                <div class="text-center w-24 flex-shrink-0">
    ...
    generarPonenciaCardHTML(ponencia) {
        ...
        return `
            <div class="flex items-start space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-${ponencia.badgeColor}-500">
                <div class="text-center w-24 flex-shrink-0">
                    <span class="text-2xl font-bold text-${ponencia.badgeColor}-600">${ponencia.horario.split(' - ')[0]}</span>
                    <span class="text-gray-500">${ponencia.horario.split(' - ')[1]}</span>
                </div>
                <div class="flex-1">
                    ...
                    <span class="text-sm font-medium text-${ponencia.badgeColor}-600 bg-${ponencia.badgeColor}-100 py-1 px-3 rounded-full">${badgeText}</span>
                </div>
                <div class="flex-shrink-0">
    ...
    generarTallerHTML(ponencia) {
        ...
        return `
            <div class="flex items-start space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-green-500">
                <div class="text-center w-24 flex-shrink-0">
    ...
    generarCartelHTML(ponencia) {
        ...
        return `
            <div class="flex items-start space-x-6 bg-white p-6 rounded-lg shadow-lg border-l-4 border-purple-500">
                <div class="flex-shrink-0">
                    <img class="h-28 w-28 rounded-full object-cover shadow-md" src="${avatar}" alt="Cartel de Resultados">
                </div>
                <div class="flex-1">
                    <div class="flex items-center justify-between mb-2">
                        <h3 class="text-lg font-bold text-gray-800">${ponencia.titulo}</h3>
                        <div class="text-right ml-4 flex-shrink-0">
                            <div class="bg-purple-50 px-3 py-1 rounded-lg">
                                <span class="text-sm font-bold text-purple-600">${ponencia.horario.split(' - ')[0]}</span>
                                <span class="text-xs text-purple-500"> - </span>
                                <span class="text-sm font-bold text-purple-600">${ponencia.horario.split(' - ')[1]}</span>
                            </div>
                        </div>
                    </div>
                    <p class="text-gray-500 text-sm mb-3">Ponente: <span class="font-semibold text-gray-700">${ponencia.ponente}</span></p>
